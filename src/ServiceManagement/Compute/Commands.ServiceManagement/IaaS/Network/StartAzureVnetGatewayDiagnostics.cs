...
namespace Microsoft.WindowsAzure.Commands.ServiceManagement.IaaS
{
    ...
    public class StartAzureVnetGatewayDiagnostics : ServiceManagementBaseCmdlet
    {
        ...
        [Parameter(Position = 3, Mandatory = true, HelpMessage = "The customer's storage account key")]
        public string CustomerStorageKey { get; set; }

        [Parameter(Position = 4, Mandatory = true, HelpMessage = "The customer's storage account name")]
        public string CustomerStorageName { get; set; }

        protected override void OnProcessRecord()
        {
            UpdateGatewayPublicDiagnostics parameters = new UpdateGatewayPublicDiagnostics()
            {
                CaptureDurationInSeconds = this.CaptureDurationInSeconds.ToString(),
                ContainerName = this.ContainerName,
                CustomerStorageKey = this.CustomerStorageKey,
                CustomerStorageName = this.CustomerStorageName,
